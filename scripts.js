...
const actualizarMensaje = () => {
  ...
  camposCompletos(espaciosLibres, celdasVacias);

};
const camposCompletos = (espaciosLibres, celdasVacias) => {
  const agregar = document.querySelector('form button');

  if(celdasVacias.length === 0){
    agregar.disabled = true;
    input.disabled = true;
    espaciosLibres.innerText = "El parqueo se encuentra lleno";
    espaciosLibres.style.color = 'red';
  }else if(celdasVacias.length === 1){
    espaciosLibres.innerText = `Tienes ${celdasVacias.length} parqueos disponibles actualmente`;
